...
export default function LabTestsPage() {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      setOrderSuccess(
        res.data?.sample_id || res.data?.id?.toString() || "submitted",
      );
      setCart([]);
      setShowCart(false);
    } catch (err: any) {
      ...
      <div className="mx-auto max-w-6xl space-y-8">
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-3xl">
              ...
              <p className="mt-3 text-sm leading-6 text-muted-foreground">
                Select the tests you need, add them to your order, and submit.
                Your order goes directly to the lab for processing.
              </p>
            </div>
          ...
          <div className="rounded-2xl border border-green-200 bg-green-50 p-6 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Your order has been submitted and is pending sample collection.
                Check your patient portal for status updates.
              </p>
            </div>
            <Button
              variant="ghost"
        ...
        {showCart && (
          <section className="rounded-2xl border border-primary/30 bg-primary/5 p-6">
            ...
            {cart.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                No tests selected yet. Browse the catalog below and click
                &ldquo;Add to Order&rdquo;.
              </p>
            ) : (
          ...
          {loading ? (
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {filteredTests.map((test) => (
                <article
                  key={test.id}
                  className="rounded-2xl border border-border/60 bg-background p-5 shadow-sm flex flex-col"
                >
                  ...
                  <p className="mt-3 text-sm leading-6 text-muted-foreground flex-1">
                    {test.description ||
                      "Laboratory test available through the SecureMed diagnostics workflow."}
                  </p>
