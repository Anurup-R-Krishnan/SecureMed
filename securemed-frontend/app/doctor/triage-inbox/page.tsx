...
import { CheckCircle2, XCircle, Inbox, Clock, User, Loader2, RefreshCw } from 'lucide-react';
...
export default function TriageInboxPage() {
    ...
    const fetchInbox = useCallback(async () => {
        ...
        try {
            ...
            toast({ title: 'Failed to load triage requests', variant: 'destructive' });
        } finally {
            setLoading(false);
        }
    }, [toast]);
    ...
    const handleAction = async (triageId: number, action: 'APPROVED' | 'DECLINED') => {
        setActioning(triageId);
        try {
            await api.post('/telemedicine/triage/approve/', { triage_id: triageId, action });
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-black tracking-tight">Pending AI Triage Requests</h1>
                    <p className="text-sm text-muted-foreground mt-1">
                        Review AI-generated patient summaries and approve or decline appointment requests.
                    </p>
                </div>
            ...
            {loading && (
                <div className="flex items-center justify-center py-24 text-muted-foreground">
                    ...
                    <span className="text-base font-medium">Loading triage requests…</span>
                </div>
            )}
            ...
            {!loading && requests.length === 0 && (
                <div className="flex flex-col items-center justify-center py-24 text-center text-muted-foreground border-2 border-dashed border-border rounded-2xl">
                    ...
                    <p className="text-lg font-semibold">All clear — no pending requests</p>
                    ...
                    {requests.map((req) => (
                        ...
                        >
                            ...
                            <CardContent className="flex-1 pt-4 pb-3">
                                ...
                                <div
                                    className="text-sm text-foreground leading-relaxed whitespace-pre-wrap bg-muted/30 rounded-xl p-3 border border-border/30 max-h-52 overflow-y-auto"
                                >
                                    {req.ai_summary}
                                </div>
                            </CardContent>
