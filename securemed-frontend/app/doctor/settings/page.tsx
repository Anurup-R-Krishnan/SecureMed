...
export default function SettingsPage() {
    ...
    const [initialProfile, setInitialProfile] = useState({ fullName: '', email: '', specialty: '' });
    ...
    useEffect(() => {
        ...
        const specialtyDisplay = user.doctor_profile?.specialization_display || user.doctor_profile?.specialization || '';
        setFullName(name);
        setEmail(user.email || '');
        setSpecialty(specialtyDisplay);
        setInitialProfile({ fullName: name, email: user.email || '', specialty: specialtyDisplay });
    ...
    const handleSaveProfile = async () => {
        ...
        try {
            ...
            setInitialProfile({ fullName: trimmedName, email: emailValue, specialty });
            ...
            toast({
                title: 'Update failed',
                description: error?.response?.data?.error || 'Could not save profile changes.',
                variant: 'destructive'
            });
        } finally {
    ...
    const handlePasswordUpdate = async () => {
        ...
        try {
            ...
            toast({
                ...
                description: error?.response?.data?.error || 'Could not update password.',
                variant: 'destructive'
            });
        } finally {
    ...
    const isDirty = fullName !== initialProfile.fullName || email !== initialProfile.email || avatarPreview !== initialAvatar;
                    ...
                    <div className="grid gap-6 md:grid-cols-2">
                        ...
                        <div className="col-span-2 md:col-span-1 bg-card border rounded-2xl p-6 flex flex-col items-center justify-center text-center gap-4 relative overflow-hidden group">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-muted-foreground">{specialty || 'Specialist'}</p>
                            </div>
                        ...
                        <div className="col-span-2 md:col-span-1 bg-card border rounded-2xl p-6 space-y-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <Input id="specialty" value={specialty} disabled className="bg-muted/50" />
                            </div>
                        </div>
                    </div>
                </TabsContent>
