...
import { appointmentService, Doctor, TimeSlot } from "@/services/appointments";
import {
  Calendar,
  Clock,
  User,
...
export default function AppointmentBooking({
  ...
  const specialties = Array.from(
    new Set(doctors.map((doc) => doc.specialty || doc.specialization)),
  ).filter(Boolean);
    ...
    try {
      ...
      if (error.response?.data) {
        ...
        if (errorMessage.toLowerCase().includes("referral")) {
          errorMessage +=
            " Try booking with a General Practitioner (Dr. Robert General) for an initial consultation.";
        }
      }
      ...
      <div className="flex items-center justify-center gap-2 mb-12">
        {steps.map((step, idx) => (
          <div key={step.id} className="flex items-center">
            <div
              className={`flex flex-col items-center gap-2 ${idx <= currentIndex ? "text-primary" : "text-muted-foreground"}`}
            >
              <div
              ...
              <span className="hidden md:inline font-bold text-[10px] uppercase tracking-tighter">
                {step.label}
              </span>
            </div>
            {idx < steps.length - 1 && (
              <div
                className={`h-0.5 w-8 md:w-20 mx-4 -mt-4 transition-all duration-500 ${idx < currentIndex ? "bg-primary" : "bg-muted"}`}
              />
            )}
          </div>
        ))}
        ...
        <Card className="p-8 border-none shadow-2xl bg-gradient-to-br from-card to-muted/20">
          ...
          <div className="grid md:grid-cols-2 gap-6 mb-8">
            <div className="relative group">
              <label className="block text-xs font-black text-muted-foreground uppercase tracking-widest mb-2 ml-1">
                Search Doctor
              </label>
          ...
          {isLoading && doctors.length === 0 ? (
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {doctors.map((doc) => (
                <Card
                  key={doc.id}
                  className={`p-6 cursor-pointer transition-all duration-300 border-2 relative overflow-hidden group ${
                ...
                >
                  {/* Decorative circle */}
                  <div
                    className={`absolute -right-4 -bottom-4 w-16 h-16 rounded-full transition-all duration-300 ${selectedDoctor?.id === doc.id ? "bg-primary/10" : "bg-muted/30 group-hover:bg-primary/5"}`}
                  />
                  ...
                  <div className="flex items-start gap-4 h-full relative z-10">
                    ...
                    <div className="flex-1 flex flex-col justify-between h-full">
                      <div>
                        ...
                        <p className="text-xs font-bold text-primary uppercase tracking-tighter mt-0.5">
                          {doc.specialty}
                        </p>
              ...
              <div>
                ...
                {selectedDate ? (
                  <div className="p-8 rounded-[32px] bg-gradient-to-br from-primary to-primary-foreground text-white shadow-2xl relative overflow-hidden group">
                    ...
                    <p className="text-[10px] uppercase font-black tracking-[0.2em] opacity-80 mb-2">
                      Confirmed Date
                    </p>
                  ...
                  <div className="p-8 rounded-[32px] border-2 border-dashed border-border flex flex-col items-center justify-center text-center">
                    ...
                    <p className="text-muted-foreground italic font-medium">
                      Pick a date from the calendar to see availability.
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="p-6 rounded-2xl bg-muted/30 border border-border">
                ...
                <ul className="space-y-3">
                  <li className="flex items-start gap-3 text-xs text-muted-foreground">
                    <div className="h-4 w-4 shrink-0 rounded bg-primary/20 text-primary flex items-center justify-center text-[8px] font-black">
                      1
                    </div>
                    <span>
                      Confirm your booking at least 24 hours before your slot.
                    </span>
                  </li>
                  <li className="flex items-start gap-3 text-xs text-muted-foreground">
                    <div className="h-4 w-4 shrink-0 rounded bg-primary/20 text-primary flex items-center justify-center text-[8px] font-black">
  ...
  if (currentStep === "time") {
    const availableCount = availableSlots.filter(
      (s) => s.isAvailable && !s.isBooked && s.slotType === "AVAILABLE",
    ).length;
    const bookedCount = availableSlots.filter((s) => s.isBooked).length;
    const surgeryCount = availableSlots.filter(
      (s) => s.slotType === "SURGERY",
    ).length;
        ...
        <Card className="p-10 max-w-5xl mx-auto border-none shadow-2xl bg-card">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12">
            {[
              ...
              >
                <div
                  className={`h-8 w-8 rounded-xl ${item.color} shadow-lg shrink-0 flex items-center justify-center text-white font-black text-xs`}
                >
                  {item.count}
                </div>
          ...
          {isLoading ? (
            <div className="flex flex-col items-center justify-center py-32 opacity-50">
              ...
              <p className="font-bold text-muted-foreground tracking-widest uppercase text-xs">
                Syncing with hospital servers...
              </p>
            </div>
          ) : (
              ...
              {availableSlots.map((slot, index) => {
                ...
                return (
                  <button
                    ...
                    className={`group relative flex flex-col items-center justify-center p-6 rounded-[24px] border-2 transition-all duration-300 ${
                      isSelected
                  ...
                  >
                    <Clock
                      className={`h-5 w-5 mb-3 ${isSelected ? "text-primary-foreground" : "text-muted-foreground"}`}
                    />
                    <span
                      className={`text-lg font-black tracking-tight ${isSelected ? "text-primary-foreground" : "text-foreground"}`}
                    >
                      {
                    ...
                    <div className="mt-2 min-h-[16px]">
                      ...
                      {isSelected && (
                        <CheckCircle className="h-4 w-4 text-primary-foreground animate-in zoom-in" />
                      )}
                    </div>
          ...
          <div
            className={`transition-all duration-500 ease-out ${selectedSlot ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12 pointer-events-none"}`}
          >
              ...
              <div className="flex flex-col lg:flex-row items-center justify-between gap-10 relative z-10">
                <div className="flex items-center gap-8">
                  ...
                  <div className="text-center lg:text-left">
                    <p className="text-primary-foreground/70 text-[10px] font-black uppercase tracking-[0.2em] mb-2">
                      Selected Time Slot
                    </p>
      ...
      <div className="space-y-6 animate-in slide-in-from-right-4 duration-500">
        ...
        <Card className="p-10 max-w-3xl mx-auto border-none shadow-2xl bg-card rounded-[40px]">
          <div className="text-center mb-10">
            ...
            <p className="text-muted-foreground mt-2">
              Please double check your appointment information.
            </p>
          </div>

          <div className="space-y-8">
            <section className="bg-muted/30 p-8 rounded-[32px] border border-border/50">
              <h3 className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em] mb-6">
                Medical Provider
              </h3>
              <div className="flex justify-between items-center pb-6 border-b border-border/50">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-bold text-primary uppercase tracking-tighter mt-1">
            ...
            <section className="bg-primary/5 p-8 rounded-[32px] border border-primary/10 relative overflow-hidden">
              <h3 className="text-[10px] font-black text-primary uppercase tracking-[0.3em] mb-6">
              ...
              <div className="flex items-center gap-8">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm font-bold text-muted-foreground mt-1 underline decoration-primary/30 decoration-2">
                    Standard Clinical Session
                  </p>
                </div>
              </div>
            </section>
            ...
            <div className="space-y-6">
              <div className="group">
                <label className="block text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] mb-3 ml-2">
              ...
              <div className="p-6 rounded-[28px] bg-amber-500/5 border-2 border-amber-500/10 flex gap-4">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-amber-800/80 leading-relaxed font-medium">
                    ...
                    <b>₹{selectedDoctor?.consultation_fee}</b> will be collected
                    at the reception upon arrival. We accept all major health
                    insurance.
                  </p>
                </div>
              </div>
        ...
        <Card className="p-12 max-w-2xl w-full text-center relative overflow-hidden shadow-[0_50px_100px_-20px_rgba(0,0,0,0.15)] rounded-[48px] border-none bg-card">
          ...
          <div className="relative z-10">
            ...
            <p className="text-muted-foreground text-lg mb-12 max-w-md mx-auto leading-relaxed font-medium">
              Your health is our priority. A secure confirmation has been sent
              via <b>SMS</b> and <b>HIPAA Email</b>.
            </p>
            ...
            <div className="flex flex-col sm:flex-row gap-6">
              <Button
                className="flex-1 h-20 rounded-[28px] font-black text-lg shadow-2xl active:scale-95 transition-all"
