...
export default function AnatomyEducationCard({
  ...
  const canvasMode =
    activeConditionId && visualization ? "condition" : "selection";
  ...
  const currentConditionPain =
    visualization && visualRegion
      ? (visualization.region_pain_levels?.[visualRegion] ?? 5)
      : null;
  const currentInterpretation =
    visualization && visualRegion
      ? (visualization.pain_interpretations?.[visualRegion] || []).find(
          (rule) => {
  ...
  useEffect(() => {
    ...
    fetchConditionCatalog("top20", role)
      ...
      .catch((e: any) => {
        if (mounted)
          setError(e?.response?.data?.error || "Unable to load conditions.");
      })
      .finally(() => {
  ...
  useEffect(() => {
    ...
    fetchRegionExplainer(activeRegion, role)
      ...
      .catch((e: any) => {
        if (mounted) {
          setExplainer(null);
          setError(e?.response?.data?.error || "Unable to load explainer.");
        }
      })
      .finally(() => {
        if (mounted) setExplainerLoading(false);
      });
    return () => {
      mounted = false;
    };
    ...
    fetchConditionVisualization(activeConditionId, role)
      ...
      .catch((e: any) => {
        if (mounted) {
          setVisualization(null);
          setVisualRegion(null);
          setError(e?.response?.data?.error || "Unable to load condition.");
        }
      })
      .finally(() => {
  ...
  const runConditionMatch = useCallback(async () => {
    ...
    setMatching(true);
    try {
      const matches = await fetchConditionMatches(
      ...
      setError(
        e?.response?.data?.error ||
          "Unable to match conditions from selected pain profile.",
      );
    } finally {
      setMatching(false);
    }
  ...
  return (
    <>
      <Card
        className={`${compact ? "p-6" : "p-8"} bg-white/5 backdrop-blur-md border-white/10`}
      >
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Interactive guidance to understand symptoms and educational
                condition patterns.
              </p>
            </div>
          </div>
              ...
              {selection.selectedRegions.length > 0 ? (
                <div className="flex flex-wrap gap-1.5">
                  {selection.selectedRegions.map((regionId) => (
                    <span
                      key={regionId}
                      className="text-xs rounded-full px-2.5 py-1 bg-white/10 border border-border/50"
                    >
                ...
                <div className="space-y-3">
                  {selection.selectedRegions.map((regionId) => {
                    ...
                    return (
                      <div
                        key={regionId}
                        className="rounded-xl border border-border/60 bg-white/5 p-3 space-y-2"
                      >
                        <div className="flex items-center justify-between">
              ...
              {selection.selectedSymptoms.length > 0 ? (
                <div className="flex flex-wrap gap-1.5">
                  {selection.selectedSymptoms.map((symptom) => (
                    <span
                      key={symptom}
                      className="text-xs rounded-full px-2 py-0.5 bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300"
                    >
                      {symptom}
                    </span>
                  ))}
                ...
                <p className="text-xs text-muted-foreground">
                  Select a region to auto-suggest symptom cues.
                </p>
              )}
            </div>
          </div>
              ...
              {visualization ? (
                <div className="space-y-4">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {visualization.regions.map((regionId) => (
                          <button
                            key={regionId}
                            onClick={() =>
                              handleConditionRegionSelect(regionId)
                            }
                            className={`text-xs rounded-full px-2.5 py-1 border font-medium transition-all ${
                          ...
                          >
                            {REGION_LOOKUP[regionId]?.label || regionId}
                            {typeof visualization.region_pain_levels?.[
                              regionId
                            ] === "number" && (
                  ...
                  {visualRegion && currentInterpretation && (
                    <div
                      className={`rounded-xl border p-3 ${showEmergencyAlert ? "bg-red-500/10 border-red-500/30" : "bg-orange-500/10 border-orange-500/30"}`}
                    >
                      <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-1">
                        Pain interpretation
                      </p>
                      <p className="text-sm font-semibold text-foreground mb-1">
                        {REGION_LOOKUP[visualRegion]?.label || visualRegion}:{" "}
                        {currentConditionPain}/10
                      </p>
                      <p className="text-xs text-muted-foreground">
                    ...
                    <div className="rounded-xl border border-red-500/40 bg-red-500/15 p-3">
                      <div className="flex items-start gap-2">
                        ...
                        <p className="text-xs text-red-200">
                          High-intensity pain in this pattern may indicate an
                          urgent condition. Seek emergency care immediately.
                        </p>
                      </div>
                    </div>
                  )}
                  ...
                  {visualization.pins.length > 0 && (
                    <div className="space-y-2">
                      ...
                      {visualization.pins.map((pin) => (
                        <div
                          key={pin.id}
                          className={`rounded-xl border p-3 space-y-0.5 ${SEVERITY_BADGE[pin.severity] || SEVERITY_BADGE.medium}`}
                        >
                          <div className="flex items-center justify-between gap-2">
                ...
                <div className="text-xs text-muted-foreground">
                  Select a suggested condition or keep exploring body regions to
                  generate guidance.
                </div>
              )}
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {explainer.common_symptoms.map((s) => (
                          <span
                            key={s}
                            className="text-xs rounded-full px-2 py-0.5 bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300"
                          >
                            {s}
                          </span>
                        ))}
                    ...
                    <div className="p-3 rounded-xl bg-amber-500/5 border border-amber-500/20">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {explainer.warning_signals.map((signal) => (
                          <span
                            key={signal}
                            className="text-xs rounded-full px-2 py-0.5 bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300"
                          >
                            {signal}
                          </span>
                        ))}
      ...
      <Dialog open={wizardOpen} onOpenChange={setWizardOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Pick a region and pain level to generate educational suggestions.
              Not a diagnosis.
            </DialogDescription>
          </DialogHeader>
