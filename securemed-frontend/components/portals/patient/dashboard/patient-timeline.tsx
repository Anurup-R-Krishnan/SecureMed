...
export default function PatientTimeline({
  ...
  const handleViewDetails = async (event: TimelineEvent) => {
    ...
    const isAppointment =
      ...
      ["appointment", "appt"].includes(normalizedType);
    const isRecord = ["record", "rec", "medical_record"].includes(
      normalizedType,
    );
    ...
    const isLab =
      ...
      normalizedType === "lab";
    const isInvoice =
      category === "billing" || ["invoice", "inv"].includes(normalizedType);
    const isMedication =
      category === "medication" ||
      ["pharmacy", "prescription", "rx"].includes(normalizedType);
    const labResultId = Number(
      event.details?.lab_result_id || event.details?.result_id || rawId,
    );
  ...
  if (loading) {
    return (
      <div
        className={`p-8 flex flex-col items-center justify-center space-y-4 min-h-[400px] ${className}`}
      >
  ...
  return (
    <Card
      className={`overflow-hidden border-none shadow-xl bg-gradient-to-br from-card to-muted/10 ${className}`}
    >
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Comprehensive view of medical history and upcoming events.
            </p>
          </div>
        </div>
          ...
          {filteredEvents.length === 0 ? (
            ...
            filteredEvents.map((event, index) => {
              ...
              return (
                <div
                  key={event.id}
                  className="relative pl-8 md:pl-10 group animate-in slide-in-from-bottom-4 duration-500"
                ...
                >
                  ...
                  {/* Timeline Node */}
                  <div
                    className={`absolute -left-[5px] md:-left-[3px] top-6 h-3 w-3 rounded-full border-2 border-background ${config.bg.replace("/10", "")} shadow-[0_0_0_4px_rgba(255,255,255,0.5)] dark:shadow-[0_0_0_4px_rgba(0,0,0,0.5)] z-10 transition-transform group-hover:scale-125 duration-300`}
                  />

                  <Card
                    className={`p-0 overflow-hidden transition-all duration-300 hover:shadow-lg hover:border-primary/20 bg-background ${config.border} border-l-4`}
                  >
                    <div className="p-5 flex flex-col md:flex-row gap-4">
                      {/* Icon Box */}
                      <div
                        className={`h-12 w-12 shrink-0 rounded-2xl ${config.bg} ${config.color} flex items-center justify-center shadow-inner`}
                      >
                        ...
                        <div className="flex flex-col md:flex-row md:items-start justify-between gap-2 mb-2">
                          <div>
                            <div className="flex items-center gap-2 mb-1">
                              <Badge
                                variant="secondary"
                                className="text-[10px] font-bold uppercase tracking-wider bg-muted/50 text-muted-foreground hover:bg-muted"
                              >
                                {config.label}
                              </Badge>
