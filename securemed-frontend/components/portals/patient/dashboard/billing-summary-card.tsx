    ...
    return (
        <Card className="p-6 bg-white/5 backdrop-blur-md border-white/10">
            ...
            <div className="space-y-4">
                {/* Outstanding Balance */}
                <div className={`p-4 rounded-xl border ${hasOutstanding ? 'bg-amber-500/5 border-amber-500/20' : 'bg-green-500/5 border-green-500/20'}`}>
                    <div className="flex items-center justify-between">
                        <div>
                            <p className="text-xs text-muted-foreground uppercase tracking-wider mb-1">
                                Outstanding Balance
                            </p>
                            <p className={`text-2xl font-bold ${hasOutstanding ? 'text-amber-400' : 'text-green-400'}`}>
                                ₹{summary.outstanding_balance.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                            </p>
                        </div>
                        {hasOutstanding ? (
                ...
                <div className="grid grid-cols-2 gap-3">
                    <div className="p-3 bg-white/5 rounded-lg">
                        ...
                        <p className="text-lg font-bold text-foreground">{summary.recent_invoices_count}</p>
                    </div>
                    <div className="p-3 bg-white/5 rounded-lg">
                        ...
                        <p className="text-sm font-semibold text-foreground">
                            {summary.last_payment_date
                                ? new Date(summary.last_payment_date).toLocaleDateString('en-US', {
                                    month: 'short',
                                    day: 'numeric'
                                })
                                : 'N/A'
