...
export default function VitalsRow({ vitals, history = [] }: VitalsRowProps) {
  // Helper to extract trend data from history - use actual database data only
    ...
    <div className="grid md:grid-cols-3 gap-4">
      ...
      <Card className="p-4 bg-white/5 backdrop-blur-md border-white/10 relative overflow-hidden group hover:bg-white/10 transition-colors">
        ...
        <div className="h-12 w-full mt-2">
          {hasHistory ? (
            <Line
              options={createChartOptions("#f43f5e", hrTrend)}
              data={createChartData(hrTrend, "#f43f5e")}
            />
          ) : (
      ...
      <Card className="p-4 bg-white/5 backdrop-blur-md border-white/10 relative overflow-hidden group hover:bg-white/10 transition-colors">
        ...
        <div className="h-12 w-full mt-2">
          {hasHistory ? (
            <Line
              options={createChartOptions("#3b82f6", sysTrend)}
              data={createChartData(sysTrend, "#3b82f6")}
            />
          ) : (
            <div className="flex items-center justify-center h-full text-xs text-muted-foreground">
              Tracking...
      ...
      <Card className="p-4 bg-white/5 backdrop-blur-md border-white/10 relative overflow-hidden group hover:bg-white/10 transition-colors">
        <div className="flex justify-between items-start mb-2">
          <div>
            ...
            <div className="flex items-baseline gap-1 mt-1">
              <span className="text-2xl font-bold text-foreground">
                {vitals.weight ? Math.round(vitals.weight * 10) / 10 : "--"}
              </span>
        ...
        <div className="h-12 w-full mt-2">
          {hasHistory ? (
            <Line
              options={createChartOptions("#10b981", weightTrend)}
              data={createChartData(weightTrend, "#10b981")}
            />
          ) : (
            <div className="flex items-center justify-center h-full text-xs text-muted-foreground">
              Tracking...
