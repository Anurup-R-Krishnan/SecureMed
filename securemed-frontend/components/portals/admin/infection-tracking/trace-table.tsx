            ...
            <div className="px-6 py-4 border-b border-border/40 flex items-center gap-3">
                ...
                <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                        Ranked by confidence so the most actionable cluster is visible first.
                    </p>
                </div>
                            ...
                            return (
                                <button
                                    ...
                                    className={`w-full text-left px-6 py-5 transition-colors ${
                                        isSelected ? 'bg-red-500/5 border-l-4 border-l-red-500 pl-5' : 'hover:bg-muted/30'
                                    }`}
                                >
                                        ...
                                        <div className="min-w-0 flex-1">
                                            <div className="flex items-center gap-2 flex-wrap">
                                                <span className="font-bold text-foreground">
                                                    {trace.source_report?.patient_name || trace.source_patient_name || trace.source_report?.report_id}
                                                </span>
                                                <ArrowRight className="h-3.5 w-3.5 text-red-500 shrink-0" />
                                                <span className="font-bold text-foreground">
                                                    {trace.target_report?.patient_name || trace.target_patient_name || trace.target_report?.report_id}
                                                </span>
                                            </div>
                                            ...
                                            <div className="mt-2 flex items-center gap-2 flex-wrap text-xs">
                                                ...
                                                <Pill className="bg-amber-500/10 text-amber-700">
                                                    {VECTOR_LABELS[trace.vector_type] || trace.vector_type}
                                                </Pill>
                                            ...
                                            <div className="mt-3 text-xs text-muted-foreground flex items-center gap-4 flex-wrap">
                                                ...
                                                <span className="uppercase tracking-wide font-bold">{trace.status}</span>
                                            </div>
                                        </div>
                                    </div>
                                </button>
        ...
        <div className="p-6 space-y-6">
            <div className="space-y-3">
                ...
                <h3 className="text-xl font-black tracking-tight text-foreground leading-tight">
                    {trace.infection_name} likely moved from {trace.source_report?.patient_name || trace.source_patient_name} to {trace.target_report?.patient_name || trace.target_patient_name}
                </h3>
            ...
            <div className="space-y-3">
                <PatientCard
                    ...
                    severity={trace.source_report?.severity_display || trace.source_report?.severity || 'Unknown'}
                    infection={trace.source_report?.infection_name || trace.infection_name}
                />
                <PatientCard
                    title="Linked case"
                    name={trace.target_report?.patient_name || trace.target_patient_name}
                    diagnosedAt={trace.target_report?.diagnosed_at}
                    severity={trace.target_report?.severity_display || trace.target_report?.severity || 'Unknown'}
                    infection={trace.target_report?.infection_name || trace.infection_name}
                />
            </div>
                ...
                <div className="rounded-2xl border border-border/60 bg-background">
                    ...
                    <div className="p-4 space-y-3">
                        {contactNarratives.map((item, index) => (
                            <div key={`${trace.trace_id}-narrative-${index}`} className="rounded-xl border border-border/50 bg-card p-4">
            ...
            <div className="rounded-2xl border border-border/60 bg-background">
                ...
                <div className="p-4 space-y-3">
                    {pathSteps.map((step, index) => (
                        <div key={`${trace.trace_id}-${index}`} className="rounded-xl border border-border/50 bg-card p-4">
                                ...
                                <div>
                                    ...
                                    {step.relationship.duration_minutes ? (
                                        <p className="mt-1 text-xs text-muted-foreground">
                                            Contact window: {step.relationship.duration_minutes} minutes
                                        </p>
                                    ) : null}
                                </div>
                                {step.timing ? (
    ...
    return (
        <div className="rounded-2xl border border-border/60 bg-background p-4">
            ...
            <div className="mt-2 space-y-1 text-sm">
                <p className="text-foreground"><span className="text-muted-foreground">Disease:</span> {infection}</p>
                <p className="text-foreground"><span className="text-muted-foreground">Diagnosed:</span> {formatDate(diagnosedAt)}</p>
                <p className="text-foreground"><span className="text-muted-foreground">Severity:</span> {severity}</p>
            </div>
        </div>
    );
}
...
function MetricCard({
    ...
    return (
        <div className="rounded-2xl border border-border/60 bg-background p-4">
            ...
            <p className={`mt-3 text-sm font-bold text-foreground ${tone ?? ''}`}>{value}</p>
        </div>
    );
}
...
function NodeBadge({ node }: { node: PathNode }) {
    ...
    return (
        <span className={`inline-flex items-center px-2.5 py-1 rounded-full font-medium ${tone}`}>
...
function Pill({
    ...
    return (
        <span className={`inline-flex items-center rounded-full px-2.5 py-1 font-bold ${className}`}>
            {children}
        </span>
    );
}
...
function buildExposureSummary(trace: InfectionTrace) {
    const source = trace.source_report?.patient_name || trace.source_report?.report_id || 'The source patient';
    const target = trace.target_report?.patient_name || trace.target_report?.report_id || 'the linked patient';
...
function getPrimaryAnchor(trace: InfectionTrace) {
    const items = Array.isArray(trace.transmission_path?.path) ? trace.transmission_path.path : [];
...
function buildContactNarratives(trace: InfectionTrace): ContactNarrative[] {
    ...
    for (let index = 0; index < items.length - 4; index += 2) {
        ...
        if (firstNode.type === 'Patient' && firstRel.relationship === 'SAW' && secondNode.type === 'Doctor' && secondRel.relationship === 'WORKED_IN' && thirdNode.type === 'Room') {
            narratives.push({
        ...
        if (firstNode.type === 'Patient' && firstRel.relationship === 'VISITED' && secondNode.type === 'Room' && secondRel.relationship === 'VISITED' && thirdNode.type === 'Patient') {
            narratives.push({
        ...
        if (firstNode.type === 'Patient' && firstRel.relationship === 'USED_EQUIPMENT' && secondNode.type === 'Equipment' && secondRel.relationship === 'USED_EQUIPMENT' && thirdNode.type === 'Patient') {
            narratives.push({
...
function describeStep(from: PathNode, relationship: PathRelationship, to: PathNode) {
...
function formatPercent(score: number) {
    ...
    if (score >= 0.5) {
        return { label, className: 'bg-red-500/10 text-red-600', textClassName: 'text-red-600' };
    }
    if (score >= 0.3) {
        return { label, className: 'bg-amber-500/10 text-amber-700', textClassName: 'text-amber-700' };
    }
    return { label, className: 'bg-slate-100 text-slate-700', textClassName: 'text-slate-700' };
}
