...
function filterGraphToTrace(
  data: GraphVisualization,
  trace: InfectionTrace | null,
): GraphVisualization {
  ...
  const draw = useCallback(
    (trace: InfectionTrace | null) => {
      ...
      for (const link of links) {
        const isHighlighted =
          highlightedIds.has(link.sourceNode.id) &&
          highlightedIds.has(link.targetNode.id);
        ctx.beginPath();
        ctx.moveTo(link.sourceNode.x, link.sourceNode.y);
        ctx.lineTo(link.targetNode.x, link.targetNode.y);
        ctx.strokeStyle = isHighlighted
          ? "#ef4444"
          : REL_COLORS[link.relationship] || "#94a3b8b3";
