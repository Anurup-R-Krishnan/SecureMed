...
import { adminService, AuditLogEntry, AuditLogFilters, AuditLogResponse } from '@/services/admin';
import {
    RefreshCw,
    FileText,
    Download,
...
const CATEGORY_COLORS: Record<string, string> = {
    ...
    consent: 'bg-amber-100 text-amber-800 dark:bg-amber-900/40 dark:text-amber-300',
    clinical: 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-300',
...
function exportCsv(logs: AuditLogEntry[]) {
    const header = 'Timestamp,Actor,Email,Action,Category,Resource Type,Resource ID,Description,IP Address';
...
export default function AuditLogViewer() {
    ...
    const updateFilter = (key: keyof AuditLogFilters, value: string | number) => {
        setFilters((prev) => ({ ...prev, [key]: value === '__all__' ? undefined : value, page: key === 'page' ? (value as number) : 1 }));
    };
    ...
    const hasActiveFilters = !!(filters.action || filters.category || filters.date_from || filters.date_to || filters.search);
