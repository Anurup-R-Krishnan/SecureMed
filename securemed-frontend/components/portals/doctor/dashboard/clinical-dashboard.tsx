...
export default function ClinicalDashboard({ todayAppts, totalPatients, loading, doctorName }: ClinicalDashboardProps) {
                ...
                <div className="flex items-center gap-5">
                    ...
                    <div>
                        <h2 className="text-2xl font-black tracking-tight text-foreground flex items-center gap-2">
                            {new Date().getHours() < 12 ? 'Good Morning,' : new Date().getHours() < 18 ? 'Good Afternoon,' : 'Good Evening,'} {doctorName || 'Doctor'}
                        </h2>
                    ...
                    */}
                    ...
                    <div className="bg-card border rounded-xl p-0 overflow-hidden">
                        <div className="p-4 border-b flex justify-between items-center">
                            ...
                            {selectedPatientId ? (
                                ...
                                <span className="text-xs text-muted-foreground">Select a patient</span>
                            )}
                        </div>
                            ...
                            {timelineEvents.length > 0 ? (
                                <div className="space-y-3 relative">
                                    {timelineEvents.slice(0, 4).map((event) => (
                                        <div key={event.id} className="rounded-lg border border-border/60 bg-white/70 px-3 py-2 text-xs">
                    ...
                    <div className="bg-card border rounded-xl p-4">
                        <div className="flex items-center justify-between mb-3">
                            <h3 className="font-bold text-sm uppercase tracking-wider text-muted-foreground">Recent Labs</h3>
                            <span className="text-xs text-muted-foreground">{overview?.recent_lab_results?.length || 0} results</span>
                        </div>
                        {overview?.recent_lab_results?.length ? (
                            <div className="space-y-2">
                                {overview.recent_lab_results.map((lab: any) => (
                                    <div key={lab.id} className="flex items-center justify-between rounded-lg border border-border/60 bg-muted/20 px-3 py-2 text-sm">
                                        <div>
                                            <p className="font-semibold text-foreground">{lab.test_name}</p>
                                            <p className="text-xs text-muted-foreground">
                                                {lab.result_value} {lab.units || ''} • {lab.flag || 'Normal'}
                                            </p>
                                        </div>
                            ...
                            <div className="text-xs text-muted-foreground">No released lab results yet.</div>
                        )}
                    </div>
