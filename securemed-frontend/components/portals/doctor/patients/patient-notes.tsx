...
export default function PatientNotes({ patient }: PatientNotesProps) {
  ...
  const handleRejectSuggestion = (suggestion: AIDiagnosisSuggestion) => {
    setAcceptedSuggestions(
      acceptedSuggestions.filter((s) => s.id !== suggestion.id),
    );
  };
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-amber-700 dark:text-amber-300">
              This consultation screen includes AI-assisted decision support. AI
              suggestions are for reference only and do not replace clinical
              judgment. The final diagnosis must always be determined by the
              attending physician.
            </p>
          </div>
        </div>
      ...
      <Card className="p-6">
        ...
        <div className="mb-4">
          ...
          <div className="mt-2 flex items-center gap-2 text-xs text-muted-foreground">
            ...
            />
            <label htmlFor="private-notes-toggle">
              Mark observations as private (hidden from patient)
            </label>
          </div>
        </div>
        ...
        <div className="mb-4">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            The final diagnosis is always under your control. AI suggestions are
            only for reference.
          </p>
        </div>
