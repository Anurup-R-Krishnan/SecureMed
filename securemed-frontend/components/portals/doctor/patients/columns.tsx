...
export const getPatientsColumns = ({
...
}: PatientColumnsProps): ColumnDef<Patient>[] => [
  {
    ...
    cell: ({ row }) => {
      ...
      const displayName =
        patient.name?.trim() || patient.id.toString() || "Unknown Patient";
      return (
        <div>
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <span
          className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ${statusColors[status || "Referred"]}`}
        >
          {status}
        </span>
      );
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return priority ? (
        <span
          className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ${priorityColors[priority]}`}
        >
          {priority}
        </span>
      ) : null;
