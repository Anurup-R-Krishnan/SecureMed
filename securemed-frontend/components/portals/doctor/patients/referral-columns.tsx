...
export const getReferralsColumns = ({
  onAccept,
  onDecline,
}: ReferralColumnsProps): ColumnDef<Referral>[] => [
  {
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <span className="text-muted-foreground">
        {row.getValue("referring_doctor_name")}
      </span>
    ),
  },
  {
    ...
    cell: ({ row }) => (
      <span
        className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ${priorityColors[row.original.priority]}`}
      >
