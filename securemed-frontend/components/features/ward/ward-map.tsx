    ...
    const fetchWardData = async () => {
      try {
        ...
        const generatedRooms = Array.from({ length: 12 }, (_, i) => {
          ...
          if (assignedPatient) {
            const firstName =
              assignedPatient.user_first_name ||
              assignedPatient.first_name ||
              "";
            const lastName =
              assignedPatient.user_last_name || assignedPatient.last_name || "";
            const displayName =
              assignedPatient.name ||
              `${firstName} ${lastName}`.trim() ||
              assignedPatient.user_email ||
              "Patient";
            ...
            return {
              ...
              acuity: (i % 5 === 0 ? "critical" : "stable") as
                | "critical"
                | "stable", // Simple logic: every 5th patient is critical
              isIsolation: false,
            };
          } else {
            return {
      ...
      <div className="p-6 bg-card rounded-xl border shadow-sm">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex gap-4 text-xs">
            ...
            <div className="flex items-center gap-1">
              <span className="w-3 h-3 bg-amber-100 border border-amber-500 rounded"></span>{" "}
              Monitoring
            </div>
            <div className="flex items-center gap-1">
              <span className="w-3 h-3 bg-slate-50 border border-slate-300 rounded"></span>{" "}
              Empty
            </div>
          </div>
        </div>
             ...
             `}
                ...
                >
                  <div className="flex justify-between items-start">
                    <span className="font-mono font-black text-lg opacity-50">
                      {room.id}
                    </span>
                    {room.isIsolation && (
            ...
            {selectedRoom?.patientId && pathname.startsWith("/admin") && (
              <Button
                onClick={() => {
                  setDetailOpen(false);
                  router.push(
                    `/admin/patients?patientId=${selectedRoom.patientId}`,
                  );
                }}
              >
              ...
              <>
                ...
                <Button
                  ...
                  onClick={() => {
                    setDetailOpen(false);
                    router.push(
                      `/doctor/records?patient_id=${selectedRoom.patientId}`,
                    );
                  }}
                >
