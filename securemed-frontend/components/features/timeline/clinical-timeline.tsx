    ...
    <div className="w-full bg-card border-t border-border p-4 fixed bottom-0 left-0 right-0 z-30 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]">
      <div className="max-w-7xl mx-auto flex flex-col gap-4">
        ...
        <div className="flex items-center justify-between">
          <div
            className={`flex items-center gap-2 font-mono font-bold text-lg ${currentValue === 100 ? "text-red-500 animate-pulse" : "text-primary"}`}
          >
        ...
        <div className="relative pt-6 pb-2">
          ...
          {events.map((event) => {
            ...
            if (eventTime < twentyFourHoursAgo || eventTime > nowTime)
              return null;

            const positionPercent =
              ((eventTime - twentyFourHoursAgo) /
                (nowTime - twentyFourHoursAgo)) *
              100;

            let color = "bg-blue-500";
            if (event.category === "medication") color = "bg-green-500";
            if (event.category === "lab" && event.status === "completed")
              color = "bg-red-500";
