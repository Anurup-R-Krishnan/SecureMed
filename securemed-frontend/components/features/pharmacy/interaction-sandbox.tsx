...
export function MedicationSandbox({ mode, patientId }: MedicationSandboxProps) {
  ...
  const [checkResult, setCheckResult] = useState<InteractionCheckResult | null>(
    null,
  );
  const [latestReport, setLatestReport] = useState<InteractionReport | null>(
    null,
  );
  ...
  const reloadReports = async () => {
    try {
      ...
      if (!report) {
        setReportNotice(
          "No interaction report yet. Generating a new report now.",
        );
  ...
  React.useEffect(() => {
    const handle = setTimeout(async () => {
      ...
      try {
        ...
        setError("");
        const names = await drugInteractionService.searchMedications(
          query.trim(),
          patientId,
        );
        setResults(names);
  ...
  const visibleFindings =
    interactionFindings.length > 0 ? interactionFindings : singleDrugFindings;
  ...
  }, [limitedVisibleFindings, checkResult?.summary]);
  const hasInteractions = interactionFindings.length > 0;
  const groupedInteractionFindings = useMemo(
    () =>
      buildFindingGroups(
        limitedVisibleFindings.filter((f) => f.combination_size >= 2),
      ),
  ...
  const handleDownloadPdf = async () => {
    try {
      ...
      const blob =
        await drugInteractionService.downloadReportPDFWithGeneration(patientId);
  ...
  const handleRegenerateReport = async () => {
    try {
      ...
      for (;;) {
        ...
        const status = await drugInteractionService.getReportJobStatus(
          job.task_id,
        );
      ...
      <div className="rounded-xl border bg-card p-4">
        ...
        <div className="mt-4 relative flex gap-2">
          ...
          <button
            type="button"
            onClick={() => {
              if (query.trim()) addMedication(query.trim());
            }}
            disabled={!query.trim()}
      ...
      {latestReport ? (
        <div className="rounded-xl border bg-blue-50/60 p-4">
          ...
          <div className="mt-2 text-xs text-muted-foreground">
            Findings: {latestReport.total_findings} | Critical:{" "}
            {latestReport.critical_count} | High: {latestReport.high_count}
          </div>
        ...
        <div className="rounded-xl border bg-amber-50/60 p-4">
          ...
          <p className="mt-2 text-xs text-muted-foreground">
            {reportNotice ||
              "No interaction report found yet for this patient."}
          </p>
        </div>
      )}
      ...
      <div className="rounded-xl border bg-card p-4 min-h-[180px]">
        ...
        {!checking && checkResult && (
          <div className="mt-3 flex flex-wrap gap-2 text-xs">
            <span className="rounded-full border bg-background px-3 py-1 font-medium">
              Interactions:{" "}
              {checkResult.interaction_findings_total ??
                interactionFindings.length}
            </span>
            <span className="rounded-full border bg-background px-3 py-1 font-medium">
              Single-drug effects:{" "}
              {checkResult.single_medication_findings_total ??
                singleDrugFindings.length}
            </span>
            ...
            {checkResult.findings_truncated ? (
              <span className="rounded-full border border-amber-200 bg-amber-50 px-3 py-1 font-medium text-amber-800">
                Showing top {checkResult.visible_findings_count} of{" "}
                {checkResult.interaction_findings_total +
                  checkResult.single_medication_findings_total}
              </span>
            ) : null}
        ...
        <AnimatePresence mode="wait">
          {!checking && limitedVisibleFindings.length === 0 ? (
            ...
            >
              Add medicines to see single-med side effects and interaction side
              effects.
            </motion.div>
          ) : (
            <motion.div
            ...
            >
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  {summary.topEffects.length > 0 && (
                    <span className="text-foreground">
                      Top effects: {summary.topEffects.join(", ")}
                    </span>
                  )}
                </div>
              </div>
              {truncatedFindings && (
                <div className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
                  Showing the top {maxVisibleFindings} findings. Refine
                  medications to narrow results.
                </div>
              )}
              {groupedInteractionFindings.length > 0 && (
                <div className="space-y-2">
                  ...
                  {SEVERITY_TIERS.map((severity) => {
                    const tierGroups = groupedInteractionFindings.filter(
              ...
              {!hasInteractions && groupedSingleDrugFindings.length > 0 && (
                <div className="space-y-2">
                  ...
                  {SEVERITY_TIERS.map((severity) => {
                    const tierGroups = groupedSingleDrugFindings.filter(
                      (g) => g.severity === severity,
                    );
                    return (
                      <SeverityTierSection
        ...
        {selected.length > 3 && (
          <div className="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-3 flex items-center gap-2">
            ...
            <span className="text-sm text-amber-800">
              Only up to 3-drug combinations are evaluated. Larger combinations
              may be incomplete.
            </span>
          </div>
        )}
        {!checking && hasInteractions && (
          <div className="mt-4 rounded-lg border border-red-200 bg-red-50 p-3 flex items-center gap-2">
            ...
            <span className="text-sm text-red-800">
              Interaction-related side effects found. Showing interaction
              findings first.
            </span>
          </div>
        )}
    ...
    <div className="rounded-lg border overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          <span
            className={`text-[11px] font-bold uppercase px-2 py-0.5 rounded border ${severityTone[severity] || severityTone.moderate}`}
          >
            {severity}
          </span>
          <span className="text-xs text-muted-foreground">
            {totalFindings} finding{totalFindings !== 1 ? "s" : ""} across{" "}
            {groups.length} combination{groups.length !== 1 ? "s" : ""}
          </span>
        </div>
        <ChevronDown
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            {group.combinationSize === 1
              ? "Single medication profile"
              : `${group.combinationSize}-drug combination`}{" "}
            • {group.count} linked findings
          </p>
        </div>
        <span
          className={`text-[11px] px-2 py-0.5 rounded border ${severityTone[group.severity] || severityTone.moderate}`}
        >
          {group.severity}
        </span>
      </div>
...
function buildFindingGroups(
  findings: InteractionCheckResult["findings"],
): FindingGroup[] {
  ...
  for (const finding of findings) {
    ...
    if (existing) {
      ...
      if (
        finding.side_effect &&
        !existing.effects.includes(finding.side_effect)
      ) {
  ...
  return Array.from(grouped.values()).sort((left, right) => {
    ...
    if (right.combinationSize !== left.combinationSize)
      return right.combinationSize - left.combinationSize;
