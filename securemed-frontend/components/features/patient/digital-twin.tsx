      ...
      <div className="flex-1 flex items-center justify-center relative">
        ...
        >
          ...
          <line
            ...
            x2="40"
            y2="150"
            className="stroke-muted-foreground/30 stroke-4 hover:stroke-primary cursor-pointer"
          ...
          {patientStatus && patientStatus["chest"]?.status === "critical" && (
            <circle
              ...
              cy="110"
              r="10"
              className="fill-red-500 animate-ping opacity-75"
            />
          )}
        </svg>
        ...
        <div className="absolute bottom-0 right-0 p-2 flex flex-col gap-2 text-xs text-muted-foreground">
          ...
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 rounded-full bg-orange-400"></div> Warning
          </div>
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 rounded-full bg-slate-300"></div> Stable
          </div>
        </div>
      </div>
            ...
            <div className="flex justify-between items-start mb-4">
              <div>
                <h4 className="font-bold text-lg">
                  {patientStatus[selectedPart].label}
                </h4>
                <span
                  className={`text-xs px-2 py-0.5 rounded-full font-medium uppercase ${
                    patientStatus[selectedPart].status === "critical"
                      ? "bg-red-100 text-red-700"
                      : patientStatus[selectedPart].status === "warning"
            ...
            <div className="space-y-4">
              <div className="bg-background p-3 rounded-md shadow-sm border">
                <p className="text-sm text-foreground">
                  {patientStatus[selectedPart].details ||
                    "No specific details reported."}
                </p>
              </div>
