...
import { TimeSlot } from '@/lib/types/availability';
import { BookingResult } from '@/lib/types/appointment';
import {
    Calendar,
    Clock,
    User,
    ...
    useEffect(() => {
        if (selectedDoctor && selectedDate) {
            ...
            const fetchAvailability = async () => {
                try {
                    const slots = await getDoctorAvailability(selectedDoctor.id, dateStr);
        ...
        try {
            ...
            if (result.success) {
                ...
                toast({
                    ...
                    description: result.message || 'Please try another slot.',
                    variant: 'destructive',
                });

                // Refresh slots if slot was taken
                if (result.error === 'SLOT_TAKEN' || result.error === 'CONCURRENCY_CONFLICT') {
                    const dateStr = `${selectedDate.getFullYear()}-${String(selectedDate.getMonth() + 1).padStart(2, '0')}-${String(selectedDate.getDate()).padStart(2, '0')}`;
                    const slots = await getDoctorAvailability(selectedDoctor.id, dateStr);
                    setAvailableSlots(slots);
                                    ...
                                    <div className="flex-1 flex flex-col justify-between h-full">
                                        ...
                                        <div className="flex items-center justify-between mt-4">
                                            <div className="flex flex-col">
                                                ...
                                                <span className="text-xl font-black text-foreground">₹{doc.fee}</span>
                                            </div>
                                            <div className="flex items-center gap-1 bg-amber-500/10 px-2 py-1 rounded-lg">
                                                <Star className="h-3 w-3 fill-amber-500 text-amber-500" />
                                                <span className="text-[10px] font-black text-amber-600">4.8</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="flex flex-col lg:flex-row items-center justify-between gap-10 relative z-10">
                                <div className="flex items-center gap-8">
                                    ...
                                    <div className="text-center lg:text-left">
                                        ...
                                        <p className="text-5xl font-black tracking-tighter">{selectedSlot?.startTime}</p>
                        ...
                        <section className="bg-muted/30 p-8 rounded-[32px] border border-border/50">
                            ...
                            <div className="flex justify-between items-center pb-6 border-b border-border/50">
                                ...
                                <div className="text-right hidden sm:block">
                                    <p className="text-lg font-black leading-tight">{patientName}</p>
                        ...
                        <section className="bg-primary/5 p-8 rounded-[32px] border border-primary/10 relative overflow-hidden">
                            ...
                            <div className="flex items-center gap-8">
                                ...
                                <div className="flex-1">
                                    <p className="text-4xl font-black tracking-tighter text-foreground">{selectedSlot?.startTime}</p>
                            ...
                            <div className="p-6 rounded-[28px] bg-amber-500/5 border-2 border-amber-500/10 flex gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-[11px] text-amber-800/80 leading-relaxed font-medium">
                                        An estimated amount of <b>₹{selectedDoctor?.fee}</b> will be collected at the reception upon arrival. We accept all major health insurance.
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-muted/30 border-2 border-border/30 rounded-[40px] p-10 mb-12 text-left relative overflow-hidden group">
                            ...
                            <div className="grid grid-cols-2 gap-10 py-8 border-y border-border/50">
                                ...
                                <div>
                                    ...
                                    <p className="font-black text-primary text-2xl">{selectedSlot?.startTime}</p>
                                </div>
                            </div>
